...
*/
	...
	if(in_array($_SERVER["HTTP_HOST"], $serverArray)) {
			define("__SERVER__", "http://".$_SERVER["HTTP_HOST"]."");
		} else {
...
*/
	...
	define("__FILE_NAME_NOEXT_", substr(__FILE_NAME__, 0, -$extInt));
/*
 ***************************************************************************
 *	Grab Directory
 ***************************************************************************
...
*/
	...
	// http://__SERVER__/assets/global/buckets
	define("__S_GLOB_BUCKETS__", __S_GLOB_ASSETS__."/global/buckets/");
	// http://__SERVER__/assets/global/icons
	define("__S_GLOB_G_ICONS__", __S_GLOB_ASSETS__."/global/icons/");
	// http://__SERVER__/assets/global/images
	define("__S_GLOB_G_IMAGES__", __S_GLOB_ASSETS__."/global/images/");
...
/*
 ***************************************************************************
 *	Set MICROSITE ASSET Constants
 ***************************************************************************
*/

/*
   **********************************************************************
   * Asset folder constants from host:
   * __MY_FILETYPE__
   * //subdomain.hmheducation.com/assets/filetype/microsite/
   **********************************************************************
*/
    $assetFolders = array("catalogs", "doc", "external", "fla",
        "flv", "icons", "images", "mov", "mp3",
        "mp4", "pdf", "ppt", "swf", "txt", "xls", "zip");

    foreach ($assetFolders as $folder) 
    {
        $const = "__MY_".strtoupper($folder)."__";
        define($const, __S_GLOB_ASSETS__."/".$folder.__DIRECTORY__);
    }
/*
   **********************************************************************
   * Asset folder constants from docroot:
   * __MY_LOCAL_FILETYPE__
   * /www/hmheducation.com/docs/assets/filetype/microsite/
   **********************************************************************
*/
    $docrootAssetFolders = array("catalogs", "doc", "external", "fla",
        "flv", "icons", "images", "mov", "mp3",
        "mp4", "pdf", "ppt", "swf", "txt", "xls", "zip");

    foreach ($docrootAssetFolders as $folder) 
    {
        $const = "__MY_LOCAL_".strtoupper($folder)."__";
        define($const, __GLOB_ASSETS__."/".$folder.__DIRECTORY__);
    }
...
*/
	if((preg_match("/\bWindows NT\b/i", __AGENT__)) || 
		...
		(preg_match("/\bSilk\b/i", __AGENT__))) {
		define("__PLATFORM__", "desktop");
