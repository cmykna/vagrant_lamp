...
<?php } ?>
		...
		<script src="<?php print __S_JS__; ?>/jquery.hoverIntent.js"></script>
<?php
	print($page->isIndexPage == 1) ? "\t\t<script src=\"".__S_JS__."/jquery.cycle.lite.js\"></script>\r\n" : NULL ?>
		<link href="<?php print __S_JS__; ?>/fancybox-1.3.4/jquery.fancybox-1.3.4.css" rel="stylesheet">
<?php $pageIsHTTPS = $_SERVER['HTTPS'] ? "s" : ""; ?>
<?php /* <link rel="canonical" href="<?php echo "http" . $pageIsHTTPS . "://" .$_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']; ?>" />*/ ?>
		<link href="<?php print __SERVER__.$_SERVER['SCRIPT_NAME']; ?>" rel="canonical">

		<?php
			print $coda->codaInclude
				 .$settings->hmhIcon
				 .$settings->favIcon	
				 .$settings->masterCSS
				 .$coda->codaSettingsScript
				 .$coverflow->CoverflowInclude
				 ."\r\n";
		?>
...
<?php
	...
	<script src="<?php print __S_JS__; ?>/desktop/form.js"></script>
	<?php endif ?>
	</head>
...
<?php } else { include $_SERVER["DOCUMENT_ROOT"]."/system/templates/mega-menu.tpl.php"; } ?>
		<a id="top">&nbsp;</a>
				...
				<?php if ($menuStyle == 1):
					...
					<nav id="top-nav">
						...
						<?php
							...
							if($page->isCampaignPage == 1) {	
								//$pattern = '/<li class="social-network"><a href="campaign-email.php" title="Email Us"><img src="[^+]+" alt="Email Us"><\/a><\/li>/';
								//$contents_topNav = preg_replace($pattern, "", $contents_topNav);
								print $contents_topNav;
								print $headerNav->topEmailLink;
							} else {
								print $contents_topNav;
							}
						?>
