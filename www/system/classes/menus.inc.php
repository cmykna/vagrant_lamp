...
*/
	public function topNavLink($str = NULL, $href = NULL, $class = NULL, $external = NULL, $gaLinkTrack = NULL) {
		...
		if(preg_match("/\bBuy Now\b/i", $this->linkStr)) {
			$this->linkStr = (string) "<img src=\"".__S_BUTTONS__."/buy-now.png\" alt=\"Buy Now\">";
		}
        ...
        $this->topNavLink[] = (string) sprintf("<li%s><a%s class=\"buy-now\" href=\"%s\"%s>%s</a></li>", $this->linkClass, $this->gaLinkTrack, $this->linkHref, $this->rel, $this->linkStr);
...
*/
	public function topNavSocialLink($type = NULL, $href = NULL, $class = NULL) {
		...
		$this->linkClass = ($class != NULL) ? (string) " class=\"".$class."\"" : (string) " class=\"\"";
		...
		$this->topNavSocialLink[] = (string) sprintf("<li%s><a rel=\"external\" href=\"%s\"><img src=\"%s\" alt=\"Follow us at %s\" /></a></li>", $this->linkClass, $this->linkHref, $this->linkIcon, $this->linkType);
 ...
 *	@var $class
 *	@param string $class
 *	@var default param: 'email'
 *
*/
	public function topEmailLink($str = NULL, $href = NULL, $class = NULL) {
		$this->linkStr = (string) $str;
		$this->linkHref = (string) stripHTTP($href);
		$this->linkClass = ($class != NULL) ? (string) " class=\"".$class."\"" : (string) " class=\"social-network\"";
		$this->topEmailLink = (string) sprintf("<li%s><a class=\"campaign-email\" href=\"%s\" title=\"%s\"><img src=\"".__S_ICONS__."/icon-email.png\" alt=\"%s\"></a></li>", $this->linkClass, $this->linkHref, $this->linkStr, $this->linkStr);
		return $this->topEmailLink;
	}
/*
 ***************************************************************************
...
/*
 ***************************************************************************
 *	Method for building out the social media navigation on the page
*/
	public function social() {
		return $this->_socialNavBuild();
	}
/*
 ***************************************************************************
...
/*
 ***************************************************************************
 *	PRIVATE METHODS FOR SOCIAL NAV
*/
	private function _socialNavBuild() {
		$j = (int) 2;	// offset default items
		$topNavSocialLink = (int) count($this->topNavSocialLink);
		$this->socialNavBuild = NULL;
		for ($j; $j < $topNavSocialLink; $j++) {
			$this->socialNavBuild .=  (string) "\r\n\t\t\t\t\t".$this->topNavSocialLink[$j];
		}
		return $this->socialNavBuild;
	}
...
*/
	public function leftNavItem($group, $str, $href = NULL, $external = NULL) {
		...
		$this->leftNavItem["$this->groupStr"][] = array("item" => $this->linkStr, "slug" => $this->linkHref, "ext" => $this->rel);
...
*/
	...
	private function _itemLink($text, $href, $rel) {
	...
	private function _leftNavBuild() {
		...
		foreach ($navGroup as $arr) {
			...
			elseif ($href != "" && $i == 1) {
				echo "<ul class=\"tabs\"><li>".$this->_itemLink($text, $href, $rel)."</li>\n";
			}
			...
			elseif ($i == count($navGroup)-1) {
				echo "<li>".$this->_itemLink($text, $href, $rel)."</li>\n</ul>\n";
			} 
